...
├── generated/pas2c/            # C source generated by pas2c (62 .c + 64 .h files)
├── dist/                       # Final WASM build output (see below)
├── gateway/src/index.js        # WebSocket ↔ TCP bridge
├── web/                        # Browser frontend (menu, quick play, lobby, game config)
...
|------|------|-------------|-------------|
| `hwengine.wasm` | 3.8MB | ✅ Yes | Compiled engine (WebAssembly binary) |
| `hwengine.js` | 2.2MB | ✅ Yes | Emscripten glue code (loads and runs the WASM) |
| `hwengine.data` | 187MB | ❌ Gitignored | Game assets (graphics, sounds, fonts, maps, Lua scripts) |
...
The `generated/` directory contains the intermediate C source produced by pas2c — see [`generated/README.md`](generated/README.md) for details.
...
The original Hedgewars client is a full Qt desktop application that handles the lobby, room management, team configuration, and game launch. I replaced all of it with ~1,600 lines of HTML/JS across four files: a lobby page, a quick-play page, a menu page with custom game setup, and a shared config module. The lobby connects to the real Hedgewars server through my WebSocket gateway, implementing the same text-based protocol the Qt client speaks.
...
- **Game config UI**: A shared config module (`gameconfig.js`) defines all 16 scheme presets, 13 weapon sets, and 22 game styles extracted from the Qt source code, with human-readable descriptions from Lua script headers. Game styles that lock specific schemes or weapons (extracted from Qt `.cfg` files) disable those dropdowns automatically. The menu page (`play.html`) lets players pick from 30 themes, choose a game style, scheme, and weapon set before launching.
...
3. First load downloads ~194MB of game assets (7MB core + ~187MB themes/maps/sounds, cached after that)
